<template>
  ...
  <drop-down :showSetting="true" :opened-by-default="false" :title="titleTile + 'Layer'" class="border p-3 border-gray-400">
    ...
    <drop-down title="Code of template" :opened-by-default="false">
      <pre class="text-left">styles: {{ JSON.stringify(styles, null, "\t") }}</pre>
...
export default {
  components: {SavingTemplates, StrokePattern, ColorPicker, DropDown, WaterDepth},
  props: {
    dataJson: {
      type: Object,
      required: true,
  ...
  computed: {
    ...
    minFeatureVal(){
      return this.dataJson.features.reduce((min, val) => {
        return min < val.properties[this.propertySelected] ? min : val.properties[this.propertySelected]
      }, 999999999)
    },
    options(){
      return {
